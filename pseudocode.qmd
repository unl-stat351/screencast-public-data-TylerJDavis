...
cubs$playoffs_app <- factor(ifelse(grepl("\\S", cubs$Playoffs),1,0))


seasons_winning <- cubs %>%
  group_by(winning_catagory) %>%
  summarise(counts = n(),playoff_count = sum(playoffs_app))

library(knitr)
kable(seasons_winning)


ggplot(cubs,aes(x = Year,y = W.L.,fill = Lg)) + geom_col()

ggplot(cubs,aes(x = Year,y = W.L.,fill = playoffs_app)) + geom_col()
...
team_stadium("Oakland Athletics")
